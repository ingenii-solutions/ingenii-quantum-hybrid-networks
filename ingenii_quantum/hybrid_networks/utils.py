...
def _roll_shape_and_strides(input_shape, input_strides, window_shape, dx=1, dy=1, dz=None):   
    '''
    ...
    '''
    if dz is not None:
        shape = input_shape[:-3] + ((input_shape[-3] - window_shape[-3]) // dz + 1,)
...
def roll_numpy(input_array, window_array, dx=1, dy=1, dz=None):   
    ...
    shape, strides = _roll_shape_and_strides(
        input_array.shape, input_array.strides, window_array.shape, dx, dy, dz)
...
def roll_torch(input_array, window_array, dx=1, dy=1, dz=None):  
    ...
    '''
    shape, strides = _roll_shape_and_strides(
        input_array.shape, input_array.stride(), window_array.shape, dx, dy, dz)
