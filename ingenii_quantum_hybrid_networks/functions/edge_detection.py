...
class EdgeDetectorBase():
    ...
    def run_box(self, box, tol=1e-3):
        ...
        qbit_list = list(product([0, 1], repeat=self.total_qb))
    ...
    def run_image_qiskit(self, data, tol=1e-3, reduce=True, verbose=False):
        ...
        Run edge detection for the whole image (Qiskit version)
            ...
            reduce (bool): Whether to reduce the dimension by half at the end of the algorithm
        ...
        if self.n_dimensions == 3:
            reverted_image_small = reverted_image_small[:,:,range(1,size_x,2),:,:]
    ...
    def run_image_torch(self, data, tol=1e-3, reduce=True, verbose=False):
        ...
        final_state_h = (
            normalized_larger @ self.hadamard_large @ self.D2n @ self.hadamard_large
        ...
        if reduce:
            size_x = data.shape[-1]
            reverted_image = reverted_image[:,:,:,:,range(1,size_x,2)][:,:,:,range(1,size_x,2),:]

            if self.n_dimensions == 3:
                reverted_image = reverted_image[:,:,range(1,size_x,2),:,:]
...
class EdgeDetector2D(EdgeDetectorBase):
    ...
    def __init__(self, size, backend='aer_simulator', shots=1000):  
        ...
        super().__init__(n_dimensions=2, size=size, backend=backend, shots=shots)
...
class EdgeDetector3D(EdgeDetectorBase):
    ...
    def __init__(self, size, backend='aer_simulator', shots=100):   
        ...
        super().__init__(n_dimensions=3, size=size, backend=backend, shots=shots)
    ...
    def run(self, data, num_filters =6, tol=1e-3, reduce=True, verbose=False):
        ...
        for i in range(num_filters):
            p = [0,1] + list(perm[i])
            if self.backend=='torch':
                data_out[:,data.shape[1]*i:data.shape[1]*(i+1),:,:,:] = self.run_image_torch(data.permute(p), tol, reduce, verbose).permute(p)
            else:
                data_out[:,data.shape[1]*i:data.shape[1]*(i+1),:,:,:] = self.run_image_qiskit(data.transpose(p), tol, reduce, verbose).transpose(p)
        return data_out